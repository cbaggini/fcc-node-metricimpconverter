...
suite("Unit Tests", function () {
  test("convertHandler should correctly read a whole number input.", function () {
    assert.strictEqual(convertHandler.getNum("6km"), 6);
  });
  test("convertHandler should correctly read a decimal number input.", function () {
    assert.strictEqual(convertHandler.getNum("6.5km"), 6.5);
  });
  test("convertHandler should correctly read a fractional input.", function () {
    assert.strictEqual(convertHandler.getNum("7/2km"), 3.5);
  });
  test("convertHandler should correctly read a fractional input with a decimal.", function () {
    assert.strictEqual(convertHandler.getNum("7.5/2km"), 3.75);
  });
  test("convertHandler should correctly return an error on a double-fraction.", function () {
    expect(convertHandler.getNum("3/2/2km")).to.be.NaN;
  });
  ...
  test("convertHandler should correctly return an error for an invalid input unit.", function () {
    assert.strictEqual(convertHandler.spellOutUnit("ng"), "");
  });
